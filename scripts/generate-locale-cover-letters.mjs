#!/usr/bin/env node
/**
 * Generates translated cover letter example MDX files for fr/de/ar.
 ...
 *
 * Usage:
 *   node scripts/generate-locale-cover-letters.mjs --lang fr [--dry-run] [--limit N]
 */
import fs from 'fs';
...
if (langIdx === -1 || !args[langIdx + 1]) {
  console.error('Usage: node scripts/generate-locale-cover-letters.mjs --lang fr|de|ar [--dry-run] [--limit N]');
  process.exit(1);
}
...
if (!['fr', 'de', 'ar', 'ja', 'it', 'ko', 'th', 'vi', 'pt', 'tr'].includes(LANG)) {
  console.error(`Unsupported language: ${LANG}. Use fr, de, ar, ja, it, ko, th, vi, pt, or tr.`);
  process.exit(1);
}

// ─── Paths ───────────────────────────────────────────────────────────────────
const EN_DIR = path.join(rootDir, 'frontend/content/cover-letter-examples');
...
// ─── Load language data ──────────────────────────────────────────────────────
const langData = await import(`./locale-data/cover-letter-${LANG}.mjs`);
...
function generateMDX(enData) {
  ...
  const body = langData.generateBody(jobTitle, category, skills, slug);
...
async function main() {
  console.log(`\n🌐 Generating ${LANG.toUpperCase()} cover letter examples...\n`);
  ...
  console.log(`📝 Will generate ${files.length} ${LANG} cover letter examples...\n`);
