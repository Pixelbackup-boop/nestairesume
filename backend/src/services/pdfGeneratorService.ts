...
// React SSR template bundle (generated by scripts/build-pdf-react-templates.mjs).
// Renders the same React components as the browser preview. Loaded lazily so a
// missing bundle degrades to the legacy string templates instead of crashing.
type ReactTemplates = typeof import('../../generated/reactTemplates');
let reactTemplates: ReactTemplates | null = null;
try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    reactTemplates = require('../../generated/reactTemplates');
} catch {
    logger.warn('React template bundle not found — using legacy string templates only');
}
...
export async function renderResumePdf(
    ...
    // PDF_REACT_TEMPLATES=true renders the shared React components (same code as
    // the browser preview); otherwise fall back to the legacy string templates.
    let templateHtml: string;
    if (
        process.env.PDF_REACT_TEMPLATES === 'true' &&
        reactTemplates?.hasReactTemplate(templateId)
    ) {
        templateHtml = reactTemplates.renderReactTemplate(templateId, data, theme, {
            ...t,
            isRtl: RTL_LOCALES.includes(locale),
        });
    } else {
        const renderTemplate = getTemplateRenderer(templateId);
        templateHtml = renderTemplate(data, theme, t, locale);
    }
