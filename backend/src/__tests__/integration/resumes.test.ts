...
describe('Resumes API Integration Tests', () => {
  ...
  describe('GET /api/v1/resumes/templates', () => {
    it('should return available templates (no auth required)', async () => {
      const response = await request(app).get('/api/v1/resumes/templates');
    ...
    it('should include premium and free templates', async () => {
      ...
      const hasPremium = response.body.some((t: any) => t.isPremium);
      const hasFree = response.body.some((t: any) => !t.isPremium);
  ...
  describe('POST /api/v1/resumes', () => {
    ...
    it('should return 429 when CV limit reached', async () => {
      const userAtLimit = createStarterUser({
        cvCreatedCount: PLAN_LIMITS.starter.cvLimit,
      });
      (mockPrisma.user.findUnique as jest.Mock).mockResolvedValue(userAtLimit);
  ...
  describe('GET /api/v1/resumes', () => {
    it('should return user resumes', async () => {
      const mockResumes = [
        createTestResume({ id: 'resume-1', userId: testUser.id }),
        createTestResume({ id: 'resume-2', userId: testUser.id }),
      ];
    ...
    it('should support pagination', async () => {
      ...
      expect(mockResumeService.getResumes).toHaveBeenCalledWith(
        testUser.id,
        10,
        5
      );
  ...
  describe('GET /api/v1/resumes/:id', () => {
    it('should return resume by id for owner', async () => {
      const mockResume = createTestResume({ id: 'resume-123', userId: testUser.id });
  ...
  describe('PUT /api/v1/resumes/:id', () => {
    it('should update resume', async () => {
      const updatedResume = createTestResume({
        id: 'resume-123',
        userId: testUser.id,
        title: 'Updated Title',
      });
