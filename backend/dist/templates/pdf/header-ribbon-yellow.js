...
const renderHeaderRibbonYellow = (data, theme, translations, locale = 'en') => {
    ...
    const ProgressBar = (label, value) => `
        <div style="margin-bottom: 10px;">
            <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                <span style="font-size: 10pt; font-weight: 500; color: #374151;">${(0, helpers_1.escapeHtml)(label)}</span>
            </div>
    ...
    const getInterestIcon = (name) => {
        ...
        if (nameLower.includes('photo'))
            return '&#128247;';
        if (nameLower.includes('novel') || nameLower.includes('book') || nameLower.includes('read'))
            return '&#128218;';
        if (nameLower.includes('ballet') || nameLower.includes('dance'))
            return '&#128131;';
        if (nameLower.includes('snowboard') || nameLower.includes('ski'))
            return '&#127938;';
        if (nameLower.includes('climb') || nameLower.includes('hik'))
            return '&#129495;';
        ...
        if (nameLower.includes('art') || nameLower.includes('paint'))
            return '&#127912;';
        if (nameLower.includes('sport') || nameLower.includes('fitness'))
            return '&#127939;';
        if (nameLower.includes('yoga'))
            return '&#129495;';
        if (nameLower.includes('swim'))
            return '&#127946;';
        if (nameLower.includes('cycle') || nameLower.includes('bike'))
            return '&#128692;';
        ...
        <div style="width: 100%; min-height: 100%; font-family: ${bodyFont}; font-size: 10pt; background-color: #ffffff; position: relative; box-sizing: border-box;">
            ...
            <header style="text-align: center; padding-top: 32px; padding-bottom: 16px;">
                ...
                <div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 12px; font-size: 10px; color: #6b7280; margin-top: 16px;">
                    ...
                    ${personalInfo.nationality ? `<span>|</span><span>${(0, helpers_1.escapeHtml)(personalInfo.nationality)}</span>` : ''}
                    ${personalInfo.idType && personalInfo.idNumber ? `<span>|</span><span>${personalInfo.idType === 'id' ? 'ID' : personalInfo.idType === 'passport' ? 'Passport' : 'License'}: ${(0, helpers_1.escapeHtml)(personalInfo.idNumber)}</span>` : ''}
                </div>
                ...
                ${(personalInfo.twitter || personalInfo.github || personalInfo.dribbble || personalInfo.behance || personalInfo.instagram) ? `
                    <div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 12px; font-size: 9px; color: #6b7280; margin-top: 8px;">
                        ${personalInfo.twitter ? `<span>Twitter: ${(0, helpers_1.escapeHtml)(personalInfo.twitter)}</span>` : ''}
                        ${personalInfo.github ? `<span>GitHub: ${(0, helpers_1.escapeHtml)(personalInfo.github)}</span>` : ''}
                        ${personalInfo.dribbble ? `<span>Dribbble: ${(0, helpers_1.escapeHtml)(personalInfo.dribbble)}</span>` : ''}
                        ${personalInfo.behance ? `<span>Behance: ${(0, helpers_1.escapeHtml)(personalInfo.behance)}</span>` : ''}
                        ${personalInfo.instagram ? `<span>Instagram: ${(0, helpers_1.escapeHtml)(personalInfo.instagram)}</span>` : ''}
                    </div>
                ` : ''}
            </header>
