...
exports.renderHeaderBlueClean = void 0;
const helpers_1 = require("./shared/helpers");
const translations_1 = require("./shared/translations");
const dateUtils_1 = require("./shared/dateUtils");
const renderHeaderBlueClean = (data, theme, translations, locale = 'en') => {
    ...
    const SectionHeader = (title, icon) => `
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 20px; border-bottom: 2px solid ${theme.primary}20; padding-bottom: 8px;">
            <div style="background-color: ${theme.primary}15; color: ${theme.primary}; padding: 6px; border-radius: 6px;">
                 ${(0, helpers_1.getIconSVG)(icon, theme.primary, 18)}
            </div>
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 12px 24px;">
                            ${contactItems.map(item => `
                                <div style="display: flex; align-items: center; gap: 8px; font-size: 12px; font-weight: 500;">
                                    ${(0, helpers_1.getIconSVG)(item.icon, '#ffffff', 14)}
                                    <span>${(0, helpers_1.escapeHtml)(item.value)}</span>
                                </div>
                            `).join('')}
                        </div>
                    </div>
                                ...
                                ${experience.map(exp => `
                                    <div style="position: relative; padding-left: 20px; border-left: 2px solid ${theme.primary}20;">
                                        ...
                                        <div style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 4px;">
                                            <h4 style="font-family: ${headingFont}; font-size: 14px; font-weight: 700; color: ${theme.heading}; margin: 0;">${(0, helpers_1.escapeHtml)(exp.title)}</h4>
                                        </div>
                                ...
                                ${languages.map(lang => `
                                    <div>
                                        <div style="display: flex; justify-content: space-between; font-size: 12px; font-weight: 600; margin-bottom: 4px;">
                                            ...
                                            <span style="color: ${theme.primary};">${(0, helpers_1.escapeHtml)(lang.proficiency)}</span>
                                        </div>
                                        <div style="height: 6px; background: #e5e7eb; border-radius: 3px; overflow: hidden;">
                                            <div style="width: ${(0, helpers_1.getLanguageLevel)(lang)}%; height: 100%; background: ${theme.primary};"></div>
                                        </div>
                                    </div>
                                `).join('')}
                            </div>
                                ...
                                <div style="margin-bottom: ${awards && awards.length > 0 ? '16px' : '0'};">
                                    ...
                                    <div style="display: flex; flex-direction: column; gap: 8px;">
                                        ${certifications.map(cert => `
                                            <div>
                                                <div style="font-weight: 600; font-size: 12px; color: ${theme.heading};">${(0, helpers_1.escapeHtml)(cert.name)}</div>
                                                <div style="font-size: 11px; color: #6b7280;">${(0, helpers_1.escapeHtml)(cert.issuer)} • ${(0, dateUtils_1.formatLocalizedDate)(cert.date, locale)}</div>
                                            </div>
                                        `).join('')}
                                    </div>
                                </div>
                            ...
                            ${awards && awards.length > 0 ? `
                                <div>
                                    ...
                                    <div style="display: flex; flex-direction: column; gap: 8px;">
                                        ${awards.map(award => `
                                            <div>
                                                <div style="font-weight: 600; font-size: 12px; color: ${theme.heading};">${(0, helpers_1.escapeHtml)(award.title)}</div>
                                                <div style="font-size: 11px; color: #6b7280;">${(0, helpers_1.escapeHtml)(award.issuer)} • ${(0, dateUtils_1.formatLocalizedDate)(award.date, locale)}</div>
                                            </div>
                                        `).join('')}
                                    </div>
                                </div>
                        ...
                        <div style="margin-bottom: 40px;">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 8px; font-size: 12px;">
                                ...
                                ${personalInfo.instagram ? `
                                    <div style="display: flex; align-items: center; gap: 8px;">
                                        ${(0, helpers_1.getIconSVG)('camera', theme.primary, 14)}
                        ...
                        <div style="margin-bottom: 40px;">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 16px;">
                                ${data.references.map(ref => `
                                    <div>
                                        <div style="font-weight: 700; font-size: 12px; color: ${theme.heading};">${(0, helpers_1.escapeHtml)(ref.name)}</div>
                                        <div style="font-size: 11px; font-style: italic; color: #4b5563;">${(0, helpers_1.escapeHtml)(ref.title)}, ${(0, helpers_1.escapeHtml)(ref.company)}</div>
                                        ${ref.email ? `<div style="font-size: 11px; color: ${theme.primary};">${(0, helpers_1.escapeHtml)(ref.email)}</div>` : ''}
                                        ${ref.phone ? `<div style="font-size: 11px; color: ${theme.primary};">${(0, helpers_1.escapeHtml)(ref.phone)}</div>` : ''}
                                    </div>
                                `).join('')}
                            </div>
                        </div>
                    ...
                    ${(personalInfo.nationality || (personalInfo.idType && personalInfo.idNumber) || personalInfo.customField) ? `
                        <div style="margin-bottom: 40px;">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 8px; font-size: 12px; color: #4b5563;">
                                ${personalInfo.nationality ? `<div><span style="font-weight: 600; color: ${theme.heading};">Nationality:</span> ${(0, helpers_1.escapeHtml)(personalInfo.nationality)}</div>` : ''}
                                ${personalInfo.idType && personalInfo.idNumber ? `
                                    <div><span style="font-weight: 600; color: ${theme.heading};">${personalInfo.idType === 'id' ? 'ID' : personalInfo.idType === 'passport' ? 'Passport' : 'License'}:</span> ${(0, helpers_1.escapeHtml)(personalInfo.idNumber)}</div>
                                ` : ''}
                                ${personalInfo.customField ? `
                                    <div style="margin-top: 8px;">
                                        <span style="font-weight: 600; color: ${theme.heading}; display: block; margin-bottom: 2px;">${(0, helpers_1.escapeHtml)(personalInfo.customFieldLabel || t.sections.additionalInfo)}</span>
...
exports.renderHeaderBlueClean = renderHeaderBlueClean;
//# sourceMappingURL=header-blue-clean.js.map