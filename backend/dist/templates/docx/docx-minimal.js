...
exports.renderDocxMinimal = renderDocxMinimal;
...
function renderDocxMinimal(data, theme, translations, locale) {
    ...
    if (pi.profileImage) {
        ...
        html += (0, docxHelpers_1.renderProfileImage)(pi.profileImage, 70, pi.imageShape || 'circle');
        html += `</td>`;
    }
    html += `<td style="vertical-align:middle;">`;
    html += `<p style="margin:0 0 2px 0;font-size:20pt;font-weight:bold;color:${primary};font-family:${docxHelpers_1.DOCX_FONTS.heading};">${(0, docxHelpers_1.escapeHtml)(pi.fullName)}</p>`;
    if (pi.jobTitle) {
        html += `<p style="margin:0;font-size:11pt;color:#666666;">${(0, docxHelpers_1.escapeHtml)(pi.jobTitle)}</p>`;
    }
    html += `</td>`;
    html += `</tr></table>`;
    html += `</td>`;
    ...
    if (pi.email)
        html += `<p style="margin:0 0 2px 0;font-size:9pt;color:#555555;">${(0, docxHelpers_1.escapeHtml)(pi.email)}</p>`;
    if (pi.phone)
        html += `<p style="margin:0 0 2px 0;font-size:9pt;color:#555555;">${(0, docxHelpers_1.escapeHtml)(pi.phone)}</p>`;
    if (pi.location)
        html += `<p style="margin:0 0 2px 0;font-size:9pt;color:#555555;">${(0, docxHelpers_1.escapeHtml)(pi.location)}</p>`;
    if (pi.website)
        html += `<p style="margin:0 0 2px 0;font-size:9pt;color:#555555;">${(0, docxHelpers_1.escapeHtml)(pi.website)}</p>`;
    if (pi.linkedin)
        html += `<p style="margin:0;font-size:9pt;color:#555555;">${(0, docxHelpers_1.escapeHtml)(pi.linkedin)}</p>`;
    html += `</td>`;
    html += `</tr></table>`;
    // Thin divider
    ...
    if (pi.summary) {
        ...
        html += `<p style="font-size:10pt;color:#444444;margin:0 0 10px 0;line-height:1.5;">${(0, docxHelpers_1.formatDescription)(pi.summary)}</p>`;
    }
    // === Experience ===
    if (data.experience?.length) {
        ...
        for (const exp of data.experience) {
            ...
            html += `<p style="margin:0 0 3px 0;font-size:10pt;color:#666666;">${(0, docxHelpers_1.escapeHtml)(exp.company)}${exp.city ? ' · ' + (0, docxHelpers_1.escapeHtml)(exp.city) : ''}</p>`;
            html += `</td>`;
            html += `<td style="vertical-align:top;text-align:right;white-space:nowrap;width:140px;">`;
            html += `<p style="margin:0;font-size:9pt;color:#999999;">${(0, docxHelpers_1.formatDateRange)(exp.startDate, exp.endDate, exp.current, t.labels.present, locale)}</p>`;
            html += `</td>`;
            html += `</tr></table>`;
            if (exp.description) {
    ...
    if (data.education?.length) {
        ...
        for (const edu of data.education) {
            ...
            html += `<p style="margin:0;font-size:10pt;color:#666666;">${(0, docxHelpers_1.escapeHtml)(edu.degree)}</p>`;
            html += `</td>`;
            html += `<td style="vertical-align:top;text-align:right;white-space:nowrap;width:140px;">`;
            html += `<p style="margin:0;font-size:9pt;color:#999999;">${(0, docxHelpers_1.formatDateRange)(edu.startDate, edu.endDate, edu.current, t.labels.present, locale)}</p>`;
            html += `</td>`;
            html += `</tr></table>`;
            if (edu.description) {
    ...
    if (data.interests?.length) {
        ...
        html += `<p style="font-size:10pt;color:#444444;">${data.interests.map(i => (0, docxHelpers_1.escapeHtml)(i.name)).join(' · ')}</p>`;
    }
    return html;
}
function sectionHeader(title, color) {
    return `<p style="margin:10px 0 5px 0;font-size:11pt;font-weight:bold;color:${color};border-bottom:1px solid #dddddd;padding-bottom:3px;letter-spacing:0.5px;">${(0, docxHelpers_1.escapeHtml)(title)}</p>`;
}
//# sourceMappingURL=docx-minimal.js.map