...
exports.checkAtsCompatibility = checkAtsCompatibility;
...
const DATE_RANGE_PATTERN = new RegExp(`(?:(?:${MONTH_PATTERN})[.,]?\\s+)?\\b(19|20)\\d{2}${DATE_SEPARATOR}${END_DATE}\\b`, "gi");
// Matches: "01/2023 - 12/2024", "1/2023 - Present"
const DATE_RANGE_NUMERIC = new RegExp(`\\b\\d{1,2}\\/\\d{4}\\s*(?:[-–—]|to)\\s*(?:\\d{1,2}\\/\\d{4}|${PRESENT_LABELS})\\b`, "gi");
// Matches: "2020-01 – Present", "2015-05 – 2020-06" (ISO YYYY-MM format)
const DATE_RANGE_ISO = new RegExp(`\\b(19|20)\\d{2}-\\d{2}\\s*[–—-]\\s*(?:(19|20)\\d{2}-\\d{2}|${PRESENT_LABELS})\\b`, "gi");
 ...
 */
function normalizeSpacedHeaders(text) {
    return text.split("\n").map((line) => {
...
// ── Category Checkers ──────────────────────────────────
function checkTextExtractability(text, pageCount) {
    const details = [];
    let score = 0;
    const maxScore = 15;
...
function checkSectionStructure(text) {
    const details = [];
    let score = 0;
    const maxScore = 20;
...
function checkExperienceParsing(text) {
    const details = [];
    let score = 0;
    const maxScore = 20;
...
function checkSkillsAndKeywords(text) {
    const details = [];
    let score = 0;
    const maxScore = 10;
    // Action verbs (4 pts)
...
// ── Main Check Function ────────────────────────────────
async function checkAtsCompatibility(buffer, mimeType) {
    let text;
    let pageCount;
    // Extract text
    if (mimeType === "application/pdf") {
        ...
        text = normalizeSpacedHeaders(await (0, resumeParserService_1.extractTextFromDocx)(buffer));
    }
    else {
        return {
            score: 0,
...
//# sourceMappingURL=atsCheckerService.js.map