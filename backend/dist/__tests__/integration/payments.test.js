...
describe('Payments API Integration Tests', () => {
    ...
    describe('POST /api/v1/payments/create-checkout', () => {
        it('should create checkout session for valid plan', async () => {
            ...
            expect(mockStripeService.createCheckoutSession).toHaveBeenCalledWith(testUser.id, testUser.email, testUser.email, 'gold');
        });
    ...
    describe('POST /api/v1/payments/create-portal', () => {
        ...
        it('should return 500 if user has no Stripe customer', async () => {
            mockStripeService.createPortalSession.mockRejectedValue(new Error('No Stripe customer ID found'));
            const response = await (0, supertest_1.default)(app_1.default)
                .post('/api/v1/payments/create-portal')
                .set('Authorization', `Bearer ${authToken}`);
            expect(response.status).toBe(testUtils_1.HTTP_STATUS.INTERNAL_ERROR);
    ...
    describe('Authorization', () => {
        ...
        it('all payment endpoints require authentication', async () => {
            for (const endpoint of endpoints) {
                const req = (0, supertest_1.default)(app_1.default)[endpoint.method](endpoint.path);
                if (endpoint.body) {
