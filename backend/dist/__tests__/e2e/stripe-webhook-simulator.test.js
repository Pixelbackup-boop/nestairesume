...
const STRIPE_CONFIGURED = env_1.config.stripeSecretKey && env_1.config.stripeSecretKey.startsWith('sk_test_');
...
function createWebhookPayload(event) {
    if (!env_1.config.stripeWebhookSecret) {
        return null;
    }
    ...
    const signature = crypto
        .createHmac('sha256', env_1.config.stripeWebhookSecret)
...
describeIfStripe('Stripe Webhook Simulator', () => {
    ...
    afterAll(async () => {
        if (testUserId) {
            try {
                await database_1.default.user.delete({ where: { id: testUserId } });
            }
            catch (e) {
    ...
    describe('checkout.session.completed', () => {
        it('should update user tier on successful checkout', async () => {
            ...
            const userBefore = await database_1.default.user.findUnique({ where: { id: testUserId } });
    ...
    describe('Subscription Tier Verification', () => {
        it('should have correct limits after STARTER subscription', async () => {
            ...
            const user = await database_1.default.user.findUnique({ where: { id: testUserId } });
    ...
    describe('invoice.paid (Monthly Renewal)', () => {
        it('should reset usage counters on invoice paid', async () => {
            ...
            const userAfter = await database_1.default.user.findUnique({ where: { id: testUserId } });
...
//# sourceMappingURL=stripe-webhook-simulator.test.js.map