...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    const alternateLanguages = hreflangAlternates(siteUrl, '/resume-io-alternative');
...
export default async function ResumeIoAlternativePage({ params }: { params: Promise<{ locale: string }> }) {
    const { locale } = await params;
    const c = getContent(locale);
        ...
        <>
            ...
            <section className="pt-32 pb-16 bg-gray-50">
                <div className="max-w-4xl mx-auto px-6 text-center">
                    <span className="text-accent-purple font-semibold tracking-wider uppercase text-sm">{c.hero.badge}</span>
                    <h1 className="text-4xl md:text-6xl font-bold mt-4 mb-6 text-gray-900">
                        {c.hero.title}<br />
                        <span className="text-accent-purple">{c.hero.titleHighlight}</span>
                    </h1>
                    <p className="text-lg md:text-xl text-gray-600 mb-8 max-w-2xl mx-auto leading-relaxed"
                       dangerouslySetInnerHTML={heroSubtitleHtml} />
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <Link href={localizedHref('/onboarding')} className="px-8 py-4 bg-accent-purple text-white font-semibold rounded-xl hover:opacity-90 transition shadow-lg shadow-purple-500/30">{c.hero.ctaPrimary}</Link>
                        ...
                        <div className="grid md:grid-cols-3 gap-4">
                            {c.problem.stats.map((stat, i) => (
                                <div key={i} className="bg-white rounded-xl p-5 border border-purple-100">
                                    <div className={`text-3xl font-bold mb-1 ${i === c.problem.stats.length - 1 ? 'text-accent-green' : 'text-purple-600'}`}>{stat.value}</div>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-white border-b border-gray-100">
                                    ...
                                    <th className="text-center p-4 font-semibold text-accent-purple">{c.comparison.oursName}</th>
                                </tr>
                            </thead>
                            <tbody>
                        ...
                        {c.comparison.rows.map((row, i) => (
                            <div key={i} className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
                                ...
                                <div className="space-y-2 text-sm">
                                    ...
                                    <div className="flex justify-between items-start gap-2">
                                        <span className="text-accent-purple font-medium shrink-0">{c.comparison.oursName}:</span>
                        ...
                        <div className="bg-purple-50 rounded-2xl p-8 border border-purple-200">
                            ...
                            <ul className="space-y-3 text-gray-700">
                                {c.recommendation.useUs.items.map((item, i) => (
                                    <li key={i} className="flex gap-3"><span className="text-accent-purple mt-0.5">&#10003;</span>{item}</li>
                                ))}
                            </ul>
                        </div>
                    </div>
            ...
            <section className="py-16 bg-white border-t border-gray-100">
                <div className="max-w-4xl mx-auto px-6 text-center">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        ...
                        <Link href={localizedHref('/resume-format')} className="px-6 py-3 bg-gray-50 text-gray-700 font-semibold rounded-xl hover:bg-gray-100 transition">{c.resumeExamples.ctaTemplates}</Link>
                    </div>
                </div>
            </section>
            ...
            <section className="py-20 bg-gray-50">
                <div className="max-w-3xl mx-auto px-6 text-center">
                    ...
                    <Link href={localizedHref('/onboarding')} className="inline-block px-10 py-4 bg-accent-purple text-white font-semibold rounded-xl hover:opacity-90 transition shadow-lg shadow-purple-500/30 text-lg">{c.bottomCta.cta}</Link>
