...
const alternateLanguages: Record<string, string> = { 'x-default': `${siteUrl}/en/europass-alternative` };
    locales.forEach((loc) => { alternateLanguages[loc] = `${siteUrl}/${loc}/europass-alternative`; });

    return {
        title: c.meta.title,
        description: c.meta.description,
        keywords: c.meta.keywords,
        openGraph: { title: c.meta.title, description: c.meta.description, type: 'article', url: `${siteUrl}/${locale}/europass-alternative` },
        twitter: { card: 'summary_large_image', title: c.meta.title, description: c.meta.description },
        alternates: { canonical: `${siteUrl}/${locale}/europass-alternative`, languages: alternateLanguages },
    };
}
...
export default async function EuropassAlternativePage({ params }: { params: Promise<{ locale: string }> }) {
    const { locale } = await params;
    const c = getContent(locale);
    ...
    const articleSchema = {
        ...
        mainEntityOfPage: { '@type': 'WebPage', '@id': `${siteUrl}/${locale}/europass-alternative` },
    };
