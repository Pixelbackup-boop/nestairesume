...
import { getContent } from '@/lib/content/zety-alternative';
...
const alternateLanguages: Record<string, string> = { 'x-default': getLocalizedUrl(siteUrl, '/zety-alternative', 'en') };
    locales.forEach((loc) => { alternateLanguages[loc] = getLocalizedUrl(siteUrl, '/zety-alternative', loc); });

    return {
        title: c.meta.title,
        description: c.meta.description,
        keywords: c.meta.keywords,
        openGraph: { title: c.meta.title, description: c.meta.description, type: 'article', url: getLocalizedUrl(siteUrl, '/zety-alternative', locale) },
        twitter: { card: 'summary_large_image', title: c.meta.title, description: c.meta.description },
        alternates: { canonical: getLocalizedUrl(siteUrl, '/zety-alternative', locale), languages: alternateLanguages },
    };
}
...
export default async function ZetyAlternativePage({ params }: { params: Promise<{ locale: string }> }) {
    const { locale } = await params;
    const c = getContent(locale);
    ...
    const articleSchema = {
        ...
        mainEntityOfPage: { '@type': 'WebPage', '@id': getLocalizedUrl(siteUrl, '/zety-alternative', locale) },
    };
