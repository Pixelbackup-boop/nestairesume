...
// Generate static params for all authors × locales
          ...
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-teal-primary font-semibold text-lg mb-3">
                {author.jobTitle} at {author.organization}
              </p>
              <p className="text-dark-teal/70 max-w-lg mb-4">
                {author.bio}
                {totalArticles > 0 && ` Author of ${totalArticles}+ career guides and articles.`}
              </p>
          ...
          <div className="max-w-4xl mx-auto px-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-dark-teal">
                Resume Guides by {author.name}
              </h2>
              <span className="text-sm text-dark-teal/60">{authorResumes.length} guides</span>
            </div>
            ...
            {authorResumes.length > 12 && (
              <div className="text-center mt-8">
                ...
                >
                  View all {authorResumes.length} resume guides
      ...
      {authorCoverLetters.length > 0 && (
        <section className="py-12 bg-white">
          <div className="max-w-4xl mx-auto px-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-dark-teal">
                Cover Letter Examples by {author.name}
              </h2>
              <span className="text-sm text-dark-teal/60">{authorCoverLetters.length} examples</span>
            </div>
            ...
            {authorCoverLetters.length > 6 && (
              <div className="text-center mt-8">
                <Link
                  ...
                  className="inline-flex items-center gap-2 text-teal-primary font-semibold hover:underline"
                >
                  View all {authorCoverLetters.length} cover letter examples
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                  </svg>
                </Link>
        ...
        <section className={`py-12 ${authorCoverLetters.length > 0 ? 'bg-light-teal' : 'bg-white'}`}>
          <div className="max-w-4xl mx-auto px-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-sm text-dark-teal/60">{authorBlogPosts.length} posts</span>
            </div>
            ...
            {authorBlogPosts.length > 6 && (
              <div className="text-center mt-8">
                <Link
                  ...
                  className="inline-flex items-center gap-2 text-teal-primary font-semibold hover:underline"
                >
                  View all {authorBlogPosts.length} blog posts
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                  </svg>
                </Link>
      ...
      <section className="py-16 bg-teal-gradient">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <p className="text-white/80 mb-8 max-w-lg mx-auto">
            Use the same strategies {author.name} recommends. Our AI builder applies expert resume techniques automatically.
          </p>
          <Link
            href="/onboarding"
