...
import { getContent } from '@/lib/content/overleaf-alternative';
...
const alternateLanguages: Record<string, string> = { 'x-default': getLocalizedUrl(siteUrl, '/overleaf-alternative', 'en') };
    locales.forEach((loc) => { alternateLanguages[loc] = getLocalizedUrl(siteUrl, '/overleaf-alternative', loc); });

    return {
        title: c.meta.title,
        description: c.meta.description,
        keywords: c.meta.keywords,
        openGraph: { title: c.meta.title, description: c.meta.description, type: 'article', url: getLocalizedUrl(siteUrl, '/overleaf-alternative', locale) },
        twitter: { card: 'summary_large_image', title: c.meta.title, description: c.meta.description },
        alternates: { canonical: getLocalizedUrl(siteUrl, '/overleaf-alternative', locale), languages: alternateLanguages },
    };
}

/* SAFE: hero.subtitle and problem.description contain only hardcoded HTML from our own content files (no user input). */
export default async function OverleafAlternativePage({ params }: { params: Promise<{ locale: string }> }) {
    const { locale } = await params;
    const c = getContent(locale);
    ...
    const articleSchema = {
        ...
        mainEntityOfPage: { '@type': 'WebPage', '@id': getLocalizedUrl(siteUrl, '/overleaf-alternative', locale) },
    };
        ...
        <>
            ...
            <section className="pt-32 pb-16 bg-gray-50">
                <div className="max-w-4xl mx-auto px-6 text-center">
                    <span className="text-accent-green font-semibold tracking-wider uppercase text-sm">{c.hero.badge}</span>
                    <h1 className="text-4xl md:text-6xl font-bold mt-4 mb-6 text-gray-900">
                        {c.hero.title}<br />
                        <span className="text-accent-green">{c.hero.titleHighlight}</span>
                    </h1>
                    <p className="text-lg md:text-xl text-gray-600 mb-8 max-w-2xl mx-auto leading-relaxed"
                       dangerouslySetInnerHTML={heroSubtitleHtml} />
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <Link href={localizedHref('/onboarding')} className="px-8 py-4 bg-accent-green text-white font-semibold rounded-xl hover:opacity-90 transition shadow-lg shadow-green-500/30">{c.hero.ctaPrimary}</Link>
                        ...
                        <div className="grid md:grid-cols-3 gap-4">
                            {c.problem.stats.map((stat, i) => (
                                <div key={i} className="bg-white rounded-xl p-5 border border-amber-100">
                                    <div className="text-3xl font-bold mb-1 text-amber-600">{stat.value}</div>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-white border-b border-gray-100">
                                    ...
                                    <th className="text-center p-4 font-semibold text-accent-green">{c.comparison.oursName}</th>
                                </tr>
                            </thead>
                            <tbody>
                        ...
                        {c.comparison.rows.map((row, i) => (
                            <div key={i} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                                ...
                                <div className="space-y-2 text-sm">
                                    ...
                                    <div className="flex justify-between items-start gap-2">
                                        <span className="text-accent-green font-medium shrink-0">{c.comparison.oursName}:</span>
                        ...
                        <div className="bg-green-50 rounded-2xl p-8 border border-green-200">
                            ...
                            <ul className="space-y-3 text-gray-700">
                                {c.recommendation.useUs.items.map((item, i) => (
                                    <li key={i} className="flex gap-3"><span className="text-accent-green mt-0.5">&#10003;</span>{item}</li>
                                ))}
                            </ul>
                        </div>
                    </div>
            ...
            <section className="py-12 bg-white border-t border-gray-100">
                <div className="max-w-4xl mx-auto px-6">
                    ...
                    {c.crossLinks.guides && c.crossLinks.guides.length > 0 && (
                        <>
                            ...
                            <div className="grid sm:grid-cols-2 gap-3">
                                {c.crossLinks.guides.map((guide, i) => (
                                    <Link key={i} href={localizedHref(guide.href)} className="flex items-center gap-3 p-3 bg-white rounded-lg hover:bg-gray-50 transition">
                                        <span className="text-accent-green">&rarr;</span>
            ...
            <section className="py-20 bg-gray-50">
                <div className="max-w-3xl mx-auto px-6 text-center">
                    ...
                    <Link href={localizedHref('/onboarding')} className="inline-block px-10 py-4 bg-accent-green text-white font-semibold rounded-xl hover:opacity-90 transition shadow-lg shadow-green-500/30 text-lg">{c.bottomCta.cta}</Link>
