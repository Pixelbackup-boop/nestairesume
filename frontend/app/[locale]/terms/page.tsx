...
import { getContent, type TermsContent } from '@/lib/content/terms-page';
...
const highlightIcons = [
  ...
  { border: 'border-accent-purple', bg: 'bg-accent-purple/20', text: 'text-accent-purple', d: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
];
              ...
              return (
                <div key={i}>
                  ...
                  <div className="feature-card rounded-xl p-6">
                    ...
                    {!section.intro && !section.items && !section.subsections && i === c.sections.length - 1 && (
                    ...
                    {/* Subsections (e.g. "What We Store" + "What We Don't Store") */}
                    {section.subsections?.map((sub, si) => {
                      // "What We Do NOT Store" subsection uses X icons
                      const isNeg = sub.subheading.includes('NOT') || sub.subheading.includes('NO ');
                      return (
                        <div key={si} className={si > 0 ? 'mt-6' : ''}>
                          <h3 className="font-semibold text-white mb-3">{sub.subheading}</h3>
                          {renderItems(sub.items, iconColor, isNeg)}
                        </div>
                      );
                    })}

                    {/* Simple items list */}
                    {section.items && renderItems(section.items, iconColor)}
                  </div>
                </div>
              );
            })}
      ...
      <section className="py-24">
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <Link href={`/${locale}/pricing`} className="inline-flex items-center gap-2 border border-gray-600 text-white px-8 py-4 rounded-xl font-semibold hover:border-accent-green transition">
              {c.cta.secondaryBtn}
            </Link>
          </div>
        </div>
