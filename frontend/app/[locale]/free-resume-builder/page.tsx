...
import { getContent } from '@/lib/content/free-resume-builder';
...
const alternateLanguages: Record<string, string> = { 'x-default': getLocalizedUrl(siteUrl, '/free-resume-builder', 'en') };
    locales.forEach((loc) => { alternateLanguages[loc] = getLocalizedUrl(siteUrl, '/free-resume-builder', loc); });

    return {
        title: c.meta.title,
        description: c.meta.description,
        keywords: c.meta.keywords,
        openGraph: { title: c.meta.title, description: c.meta.description, type: 'article', url: getLocalizedUrl(siteUrl, '/free-resume-builder', locale) },
        twitter: { card: 'summary_large_image', title: c.meta.title, description: c.meta.description },
        alternates: { canonical: getLocalizedUrl(siteUrl, '/free-resume-builder', locale), languages: alternateLanguages },
    };
}
...
export default async function FreeResumeBuilderPage({ params }: { params: Promise<{ locale: string }> }) {
    const { locale } = await params;
    const c = getContent(locale);
    ...
    const softwareAppSchema = {
        ...
        aggregateRating: { '@type': 'AggregateRating', ratingValue: '4.8', ratingCount: '4521' },
    };
    const articleSchema = {
        ...
        author: { '@type': 'Person', name: 'Emily Watson', url: getLocalizedUrl(siteUrl, '/about/emily-watson', locale), jobTitle: 'Career Development Specialist' },
        publisher: { '@type': 'Organization', name: 'Best AI Resume', url: siteUrl, logo: { '@type': 'ImageObject', url: `${siteUrl}/logo.png` } },
        mainEntityOfPage: { '@type': 'WebPage', '@id': getLocalizedUrl(siteUrl, '/free-resume-builder', locale) },
    };
        ...
        <>
            ...
            <section className="pt-32 pb-20 bg-gradient-to-b from-green-50 to-white">
                <div className="max-w-4xl mx-auto px-6 text-center">
                    ...
                    <h1 className="text-4xl md:text-6xl font-bold mb-6 text-gray-900 leading-tight">
                        {c.hero.title}<br />
                        <span className="text-green-600">{c.hero.titleHighlight}</span>
                    </h1>
            ...
            <section id="whats-free" className="py-20 bg-white">
                <div className="max-w-5xl mx-auto px-6">
                    ...
                    <div className="text-center mt-10">
                        <Link href={localizedHref('/onboarding')} className="inline-block px-8 py-4 bg-green-600 text-white font-semibold rounded-xl hover:bg-green-700 transition">
                            {c.features.cta}
                        </Link>
                    </div>
                </div>
                            ...
                            <table className="w-full border-collapse">
                                <thead>
                                    <tr className="bg-white border-b border-gray-200">
                                        ...
                                        <th className="text-center p-4 font-semibold text-green-600">{c.comparison.oursName}</th>
                                ...
                                <div key={i} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                                    ...
                                    <div className="space-y-2 text-sm">
                                        <div className="flex justify-between">
                                            <span className="text-green-600 font-medium">{c.comparison!.oursName}:</span>
            ...
            <section className="py-20 bg-white">
                <div className="max-w-5xl mx-auto px-6">
                    <div className="text-center mb-14">
                        ...
                        <p className="text-gray-600">{c.howItWorks.subtitle}</p>
                    </div>
            ...
            {c.trust && (
                <section className="py-16 bg-green-50 border-y border-green-100">
                    <div className="max-w-4xl mx-auto px-6 text-center">
                        <h2 className="text-2xl font-bold text-gray-900 mb-8">{c.trust.title}</h2>
                    ...
                    <div className="max-w-5xl mx-auto px-6 text-center">
                        ...
                        <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-4 mb-10">
                            {c.templates.styles.map((style) => (
                                <div key={style} className="p-4 bg-gray-50 rounded-xl border border-gray-100 hover:border-green-200 hover:bg-green-50 transition">
                                    <p className="font-medium text-gray-900">{style}</p>
                                    <p className="text-xs text-green-600 mt-1">Free</p>
                                </div>
                            ))}
                        </div>
                        <Link href={localizedHref('/templates')} className="inline-block px-6 py-3 bg-white border border-gray-200 text-gray-700 font-semibold rounded-xl hover:bg-gray-50 transition">
                            {c.templates.cta}
                        </Link>
                    </div>
                </section>
                ...
                <div className="max-w-4xl mx-auto px-6">
                    ...
                    <div className="grid sm:grid-cols-3 gap-3 mb-8">
                        {c.crossLinks.items.map((item, i) => (
                            <Link key={i} href={localizedHref(item.href)} className="p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition text-center border border-gray-100">
