...
import { gdocsTemplates, GdocsTemplate } from '@/lib/templates/gdocsTemplates';
...
// Google Docs template preview component
function GdocsTemplatePreview({ template }: { template: GdocsTemplate }) {
    ...
    if (template.layout === 'sidebar') {
        // Compact sidebar layout (light background sidebar)
        return (
            <div className="w-full h-full bg-white rounded-lg overflow-hidden">
                <div className="h-full flex">
                    <div className="w-[38%] h-full p-2 flex flex-col" style={{ backgroundColor: '#f0fafa' }}>
                        <div className="w-10 h-10 rounded-full mx-auto mb-1.5 bg-gray-200" />
                        <div className="space-y-0.5 text-[4px] mb-1.5" style={{ color: '#1a3a3a' }}>
                            <p className="truncate">{data.email}</p>
                            <p>{data.phone}</p>
                            <p>{data.location}</p>
                        </div>
                        <p className="text-[5px] font-semibold mb-0.5" style={{ color: accent }}>SKILLS</p>
    ...
    // Clean / Elegant layout (classic single-column with centered header)
    if (template.id === 'gdocs-elegant') {
        return (
            <div className="w-full h-full bg-white rounded-lg overflow-hidden p-2">
                <div className="text-center mb-1.5 pb-1.5" style={{ borderBottom: `1px solid ${accent}` }}>
                    <div className="w-8 h-8 rounded-full mx-auto mb-1 bg-gray-200" />
                    <h3 className="text-[9px] font-bold" style={{ color: accent, fontFamily: 'Georgia, serif' }}>{data.name}</h3>
                    <p className="text-[5px] italic" style={{ color: textMuted }}>{data.title}</p>
                ...
                <p className="text-[3px] mb-1.5 leading-relaxed line-clamp-2" style={{ color: textMuted }}>{data.summary}</p>
                <p className="text-[5px] font-semibold mb-0.5" style={{ color: accent, fontFamily: 'Georgia, serif' }}>EXPERIENCE</p>
                {data.experience.slice(0, 2).map((exp, i) => (
                    <div key={i} className="mb-0.5">
                        <div className="flex justify-between">
                            <p className="text-[4px] font-medium" style={{ color: textDark }}>{exp.role}</p>
                            <p className="text-[3px] italic" style={{ color: textMuted }}>{exp.years}</p>
                        </div>
                        <p className="text-[3px]" style={{ color: textMuted }}>{exp.company}</p>
                    </div>
                ))}
    ...
    // Default: Clean layout (Google blue)
    return (
        <div className="w-full h-full bg-white rounded-lg overflow-hidden p-2">
...
export default function GdocsOnboardingPage() {
    ...
    const handleFinish = () => {
        router.push(`/gdocs-builder?template=${formData.selectedTemplate}`);
    };
                            ...
                            {gdocsTemplates.map((template) => (
                                ...
                                >
                                    <div className="aspect-[3/4] bg-white p-2">
                                        <GdocsTemplatePreview template={template} />
                                    </div>
